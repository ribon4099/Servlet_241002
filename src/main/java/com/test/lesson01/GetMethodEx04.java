package com.test.lesson01;

import java.io.IOException;
import java.io.PrintWriter;

import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

@WebServlet("/lesson01/ex04")
public class GetMethodEx04 extends HttpServlet{
	
	@Override
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		
		response.setCharacterEncoding("utf-8");
		response.setContentType("text/html");
		
		// request params (F8: 디버깅 종료)
		
		String userId = request.getParameter("userId");  // name="userId" 여기거A 가져온다
		String name = request.getParameter("name");  // name="userId" 여기거A 가져온다
		String birth = request.getParameter("birth");  // name="userId" 여기거A 가져온다
		String email = request.getParameter("email");  // name="userId" 여기거A 가져온다
		
		PrintWriter out = response.getWriter();
		out.print("<html><head><title>회원정보</title></head><body>");
		out.print("<table border=1>");
		out.print("<tr><th>아이디</th><td>" + userId + "</td></tr>");
		out.print("<tr><th>이름</th><td>" + name + "</td></tr>");
		out.print("<tr><th>생년월일</th><td>" + birth + "</td></tr>");
		out.print("<tr><th>이메일</th><td>" + email + "</td></tr>");
		out.print("</body></html>");
		
		/*out.print("<html><head><title>회원정보</title></head><body>");
		out.print("아이디" + userId + "<br>");
		out.print("이름" + name + "<br>");
		out.print("생년월일" + birth + "<br>");
		out.print("email" + email + "<br>");
		out.print("</body></html>");*/
		
		
	}

}
